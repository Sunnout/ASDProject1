...
for node in $(oarprint host); do
    $node 'docker kill $(docker ps -q)' &
done

wait

for node in $(oarprint host); do
    $node "docker swarm leave -f" 
done
...
for node in $(oarprint host); do
  if [ $node != $host ]; then
    $node "docker swarm join --token $JOIN_TOKEN $host:2377"
  fi
done

echo "Rebuilding image"
for node in $(oarprint host); do
	$node "cd $(pwd);  docker build --rm -t asdproj ." &
done
...
node=$(nextnode 0)
 -n $node docker exec -d node-00 ./start.sh 0 $user "$@"
...
for i in $(seq 01 $(($nNodes - 1))); do
  ...
  if [ $((($i + 1) % 10)) -eq 0 ]; then
   ...
   -n $node docker exec -d node-${ii} ./start.sh $i $user contact=node-00:10000 "$@"
  sleep 1
done
echo ""
