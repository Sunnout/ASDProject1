...
public class EagerPushBroadcast extends GenericProtocol {
    ...
    private void uponEagerPushMessage(EagerPushMessage msg, Host from, short sourceProto, int channelId) {
        ...
        if (received.add(msg.getMid()) ) {
            ...
            List<Host> sample = getNeighboursSample(from);
    ...
    private List<Host> getNeighboursSample(Host from){
        int numberNeighbours = neighbours.size();
        int fanout;
        List<Host> sample = new ArrayList();
        Host[] hosts = (Host[])neighbours.toArray();

        if(numberNeighbours > 1) {
            fanout = (int)Math.ceil(Math.log(numberNeighbours));

            Random rand = new Random();
            Set<Integer> sentIndexes = new HashSet();

            while (sentIndexes.size() != fanout) {
                int ind = rand.nextInt(numberNeighbours);
                Host host = hosts[ind];

                //TODO necessario remover o from?
                if (!host.equals(from) && sentIndexes.add(ind))
                    sample.add(host);
            }
        }
